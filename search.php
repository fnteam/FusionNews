 ...
 * @version $Id: search.php 393 2012-02-10 22:37:14Z xycaleth $
 *
...
$keywords = ( isset ($GVARS['fn_search_keywords']) ) ? $GVARS['fn_search_keywords'] : '';

/**
 * Whether to keep this inclusion static or not.
 * @global bool $fn_static
 */
if ( !isset ($fn_static) )
{
    $fn_static = false;
...
if ( $keywords === '' || $fn_static )
{
    $qs = clean_query_string();
    parse_str (html_entity_decode ($qs), $query_string);

    $query_string['fn_mode'] = 'search';
    
    echo '<form method="get" action="?' . $qs . '">';
...
<table style="width:75%">
    <tr>
        ...
        <td><input type="submit" value="{$lang['srch8']}" /></td>
    </tr>
</table>
html;
    }
...
<table style="width:75%">
	<tr>
		<td><label for="fn_search_keywords">{$lang['srch1']}</label></td>
		<td><input type="text" name="fn_search_keywords" id="fn_search_keywords" size="20" /></td>
	</tr>
	<tr>
		<td valign="top"><label for="fn_search_author">{$lang['srch0']}</label></td>
		<td>$author_list</td>
	</tr>
	<tr>
		<td valign="top"><label for="category">{$lang['srch2']}</label></td>
		<td>
			$category_list
		</td>
	</tr>
	<tr>
		<td valign="top"><label for="fn_search_from_num">{$lang['srch3']}</label></td>
		<td>
			<input type="checkbox" name="fn_search_all" checked="checked" onclick="javascript:updateFields(this.checked, this.form);" value="1" /> {$lang['srch4']}<br />
			<input type="text" name="fn_search_from_num" id="fn_search_from_num" disabled="disabled" maxlength="3" size="3" /> <select name="fn_search_from_type" disabled="disabled"><option value="0">{$lang['srch5']}</option><option value="1">{$lang['srch6']}</option><option value="2">{$lang['srch7']}</option></select>
		</td>
	</tr>
	<tr>
		<td colspan="2" style="text-align:center"><input type="submit" value="{$lang['srch8']}" />&nbsp;<input type="reset" value="{$lang['ind16']}" /></td>
	</tr>
</table>
...
{
	$authors = ( isset ($PVARS['fn_search_author']) ) ? $PVARS['fn_search_author'] : array();
	$categories = ( isset ($PVARS['category']) ) ? $PVARS['category'] : array();
	$search_all = isset ($PVARS['fn_search_all']);
	$from_num = ( isset ($PVARS['fn_search_from_num']) ) ? (int)$$PVARS['fn_search_from_num'] : 0;
	$from_type = ( isset ($PVARS['fn_search_from_type']) ) ? (int)$PVARS['fn_search_from_type'] : 0;
	...
	else
	{
		...
		else
		{
			...
			foreach ( $file as $toc_line )
			{
				...
				$shortnews = format_message ($shortnews, false, $config['bb'], $config['smilies'], $config['wfpost'], true);
				$fullnews = format_message ($fullnews, false, $config['bb'], $config['smilies'], $config['wfpost'], true);
...
unset ($fn_simple_search, $fn_static);
