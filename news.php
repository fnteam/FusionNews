 ...
 * @version $Id: news.php 393 2012-02-10 22:37:14Z xycaleth $
 *
...
$fn_mode = ( isset ($GVARS['fn_mode']) ) ? $GVARS['fn_mode'] : '';
 ...
 */
$fn_news_per_page = ( isset ($fn_news_per_page) ) ? (int)$fn_news_per_page : $config['numofposts'];
/**
 * Whether or not to use pagination
 * @global bool $fn_pagination
 */
$fn_pagination = ( isset ($fn_pagination) ) ? (bool)$fn_pagination : (bool)$config['news_pagination'];
/**
 ...
 */
$fn_date_order = ( isset ($fn_date_order) ) ? $fn_date_order : ($config['flip_news'] ? 'asc' : 'desc');
if ( $fn_date_order != 'asc' && $fn_date_order != 'desc' )
{
    $fn_date_order = $config['flip_news'] ? 'asc' : 'desc';
}
...
$valid_posts = get_posts_filtered (-1, -1, $fn_author, $fn_category);
...
if ( sizeof ($valid_posts) > 0 )
{
	...
	if ( $fn_pagination )
	{
		...
		if ( $max_pages > 1 )
		{
			...
			if ( $config['news_pagination_arrows'] )
			{
				if ( ($fn_page - 1) >= 1 )
				{
					$pagination .= '<a href="?fn_page=' . ($fn_page - 1) . $other_qs . '" class="fn-news-pagination-prev">' . $config['news_pagination_prv'] . '</a>&nbsp;';
				}
				else
				{
					$pagination .= $config['news_pagination_prv'] . '&nbsp;';
				}
			}

			if ( $config['news_pagination_numbers'] || (!$config['news_pagination_numbers'] && !$config['news_pagination_arrows']))
			{
			...
			if ( $config['news_pagination_arrows'] )
			{
				...
				{
					$pagination .= '&nbsp;<a href="?fn_page=' . ($fn_page + 1) . $other_qs . '" class="fn-news-pagination-next">' . $config['news_pagination_nxt'] . '</a>';
				}
				else
				{
					$pagination .= '&nbsp;' . $config['news_pagination_nxt'];
				}
			}
		}
	}
	...
	{
		echo '<div style="text-align:' . $fn_page_align . '" class="fn-news-pagination-top">' . $pagination . '</div>';
	}
	...
	if ( $config['post_per_day'] )
	{
		...
		{
			$news_display = '';
			$display = replace_masks ($temp, array ('date' => date ($config['ppp_date'], $key)));
	...
	if ( $fn_pagination && $max_pages > 1 )
	{
		echo '<div style="text-align:' . $fn_page_align . '" class="fn-news-pagination-bottom">' . $pagination . '</div>';
	}
}
