 ...
 * @version $Id: common.php 393 2012-02-10 22:37:14Z xycaleth $
 *
...
if ( !is_file (FNEWS_ROOT_PATH . 'languages/' . $config['language'] . '.lang.php') )
{
    include_once FNEWS_ROOT_PATH . 'languages/' . DEFAULT_LANGUAGE . '.lang.php';
}
else
{
    include_once FNEWS_ROOT_PATH . 'languages/' . $config['language'] . '.lang.php';
}
...
/**
 * Combined array of $_POST which have been cleaned to be made safe
 * @global array $PVARS
 * @see clean_value()
 * @see clean_key()
 */
...
/**
 * Combined array of $_GET which have been cleaned to be made safe
 * @global array $GVARS
 * @see clean_value()
 * @see clean_key()
 */
...
foreach ( $_GET as $k => $v )
{
	if ( is_array ($v) )
	{
		...
		foreach ( $v as $_k => $_v )
		{
			$GVARS[$clean_k][clean_key ($_k)] = clean_value ($_v);
		}
	}
	else
	{
		$GVARS[clean_key ($k)] = clean_value ($v);
	}
}
...
foreach ( $_POST as $k => $v )
{
	if ( is_array ($v) )
	{
		...
		foreach ( $v as $_k => $_v )
		{
			$PVARS[$clean_k][clean_key ($_k)] = clean_value ($_v);
		}
	}
	else
	{
		$PVARS[clean_key ($k)] = clean_value ($v);
	}
}
